...
const ReviewCard = ({ review }) => {
  ...
  return (
    <div
      className="flex-none flex flex-col items-center border-2 border-primaryGreen w-60 md:w-96 py-10 px-5 rounded-xl shadow-lg group transition-all duration-500"
      onClick={showAllCommentText}
      onMouseLeave={() => setIsShowing(false)}
    >
      <div className="flex flex-col md:flex-row gap-3 items-center mb-3">
        <img
          className="w-1/3 md:w-1/5 shadow-lg border-2 border-primaryGreen rounded-full"
          src={profilePng}
          alt="User"
        />
        <div>
